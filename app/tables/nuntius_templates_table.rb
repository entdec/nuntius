...
class NuntiusTemplatesTable < ActionTable::ActionTable
  ...
  def scope
    ...
    @scope = Nuntius::Template.visible.select('nuntius_templates.*, (select count(id) from nuntius_messages where nuntius_messages.template_id = nuntius_templates.id) as message_count') if params[:order_field_name] = 'message'
  end
end
